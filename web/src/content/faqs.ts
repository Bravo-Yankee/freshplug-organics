...
export interface Faq {
  /** Added for Phase 1 (Supabase) — the legacy source data had no id; assigned sequentially here to match the seeded `faqs` table. */
  id: number;
...
// reads go through `getFaqs()` in `@/lib/data/faqs`, which queries the
// `faqs` table. Edit FAQs in Supabase Studio, not here.
